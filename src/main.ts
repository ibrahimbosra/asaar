...
function parseProduct(id: string, data: Record<string, unknown>): Product | null { const name = typeof data.name === 'string' ? data.name.trim() : ''; const price = typeof data.price === 'number' && Number.isFinite(data.price) && data.price > 0 ? data.price : Number.NaN; if (!name) return null; const storedNumber = typeof data.number === 'number' && Number.isInteger(data.number) && data.number > 0; const categoryId = typeof data.categoryId === 'string' && data.categoryId.trim() ? data.categoryId : null; return { id, name, price, number: storedNumber ? data.number as number : stableNumber(id), storedNumber, categoryId, isFavorite: data.isFavorite === true } }
...
function updateProductList() {
  ...
  list.querySelectorAll<HTMLButtonElement>('[data-favorite]').forEach((button) => button.addEventListener('click', (event) => { event.stopPropagation(); void toggleFavorite(button.dataset.favorite!) }))
...
async function toggleFavorite(id: string) {
  if (!currentUser || !settings.favoritesEnabled) return
...
function renderInstallButton() {
  ...
  button.innerHTML = `${svg('download')}<span>تثبيت التطبيق</span>`
...
const modal = (content: string) => { const element = document.createElement('div'); element.className = 'modal-backdrop'; element.innerHTML = `<section class="modal">${content}</section>`; element.addEventListener('click', (event) => { if (event.target === element) element.remove() }); document.body.append(element); element.querySelector<HTMLElement>('.rate-field small')?.replaceChildren(document.createTextNode('كل 1 دولار أمريكي كم ليرة سورية يساوي؟')); const applyCategoryPlaceholder = () => { const input = element.querySelector<HTMLInputElement>('#category-form input[name="name"]'); if (!input) return false; input.setAttribute('placeholder', 'اسم الفئة مثال: دواليب'); return true }; if (!applyCategoryPlaceholder()) { const observer = new MutationObserver(() => { if (applyCategoryPlaceholder()) observer.disconnect() }); observer.observe(element, { childList: true, subtree: true }) } return element }
